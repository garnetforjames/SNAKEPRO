...
void welcome(void)
{
	...
	while (1) {
		...
		*/
		...
		switch (option) {
		case '1':
			fmap = fopen("map_0.txt", "r");//1번 QUICK GAME을 선택하였을 경우 일반 맵인 0번 맵파일을 열어 게임 실행
			break;
		case '2':
			system("cls");
			goprint(4, 4, "Welcome to the Snake by Lst");
			goprint(6, 6, "Choose by number:\n\n");
			printf("1. Cave\n2. Hell\n3. <EMPTY>\n4. Your own map\n5. Back");//1번 CAVE맵, 2번 HELL맵, 3번 EMPTY, 4번 your own map 5번 뒤로가기
			map_choice = _getch();//MAP GAME모드 선택 후 MAP을 선택하는 키 입력
			switch (map_choice) {
			case '1':
			...
			break;
		case '3'://3번을 눌러 Revise level , 게임 난이도 설정 모드로 들어간다. 1~9까지의 수를 입력하여 플레이어의 속도를 조절
			system("cls");
			goprint(4, 4, "Welcome to the Snake by Lst");
			goprint(6, 6, "Start by number:\n\n");
			...
			option = '0';
			break;
		case '4'://4번을 입력하면 EXIT로 프로그램이 종료 
			exit(0);
		default:
			option = '0';
		}
...
void show_tips(void)//함수 이름을 적절하게 사용한 것 같다.
{
	gotoxy(margin, 3);// m
	...
	//간결하게 사용자의 컨트롤 정보와 게임 정보를 작성하였으나, 한번에 파악하기 힘듬/눈에 잘 띄지 않음. 
}
