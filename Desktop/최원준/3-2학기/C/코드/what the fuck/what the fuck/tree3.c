...
Node* delete_AVL(Node* root, int data)
{
    ...
    if(root->data > data)
        {root->leftChild = delete_AVL(root->leftChild, data);
	//	root=rebalance(&root);	
	}	
    else if(root->data < data){
        root->rightChild =delete_AVL(root->rightChild, data);
	//	root=rebalance(&root);		
	}
    else
    {
        ...
        if((root->rightChild != NULL) && (root->leftChild != NULL))
        {
            ...
            root->rightChild = delete_AVL(root->rightChild, tNode->data);
		//	root=rebalance(&root);	
		}
        else
        {
